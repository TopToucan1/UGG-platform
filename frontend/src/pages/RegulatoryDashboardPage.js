...
function KPI({ label, value, sub, color, icon: Icon, large, tip }) {
  return (
    ...
    <div data-testid="regulatory-dashboard" className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-heading text-2xl font-bold tracking-tight flex items-center" style={{ color: '#E8ECF1' }}>Regulatory Compliance Dashboard<InfoTip label="Regulatory Compliance Dashboard" description="State-level view for regulators and compliance officers: revenue, tax collection, software integrity, statutory field coverage, and distributor scorecards — everything the state gaming commission needs." /></h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[9px] uppercase tracking-widest flex items-center" style={{ color: '#6B7A90' }}>Overall Compliance<InfoTip description="Composite score blending integrity pass rate, statutory field coverage, EFT sweep status, and exception load. 90+ is healthy, below 70 needs action." /></div>
      ...
      <div className="grid grid-cols-8 gap-2" data-testid="reg-kpi-strip">
        ...
        <KPI label="30d Tax" value={fmt(d.revenue?.grand_tax)} color="#F5A623" icon={Receipt} tip="Tax the state is owed on 30 days of NOR. Should match EFT sweeps." />
        <KPI label="Integrity" value={`${d.integrity_compliance?.pass_rate}%`} color={d.integrity_compliance?.pass_rate >= 99 ? '#00D4AA' : '#FF3B30'} icon={ShieldCheck} tip="Pass rate on machine software signature checks — confirms firmware hasn't been tampered with. State typically requires 99%+." />
        <KPI label="Statutory" value={`${d.statutory_compliance?.rate}%`} color={d.statutory_compliance?.rate >= 99 ? '#00D4AA' : '#F5A623'} icon={SealCheck} tip="Share of events that carry every mandatory state-filing field (license, county, software signature, etc.)." />
        <KPI label="Exceptions" value={d.exceptions?.total_active} color="#FF3B30" icon={Warning} sub={`${d.exceptions?.critical} critical`} tip="Active machine alerts across the estate. Critical must be handled immediately." />
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-8 rounded border p-4" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-revenue-chart">
          <div className="text-[11px] uppercase tracking-wider mb-3 font-medium flex items-center" style={{ color: '#6B7A90' }}>State Revenue & Tax Collection (30 Days)<InfoTip description="Daily Coin In, NOR, and tax accrual aggregated across every distributor in the state." /></div>
        ...
        <div className="col-span-4 space-y-3">
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-statutory-gauge">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] uppercase tracking-wider font-medium flex items-center" style={{ color: '#6B7A90' }}>Statutory Field Coverage<InfoTip description="How many events have all the data the state requires (license, operator, county, software signature, serial)." /></span>
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-integrity-gauge">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] uppercase tracking-wider font-medium flex items-center" style={{ color: '#6B7A90' }}>Software Integrity<InfoTip description="Verification that each machine's running firmware signature matches the lab-approved version. Failures suggest tampering." /></span>
          ...
          {/* EFT */}
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }}>
            <div className="text-[10px] uppercase tracking-wider font-medium mb-2 flex items-center" style={{ color: '#6B7A90' }}>EFT Sweep Status<InfoTip description="Electronic Funds Transfer files that carry taxes and fees from operator accounts to the state. NACHA = the US bank file format these must follow." /></div>
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: d.buffer_risk?.agents_offline > 0 ? '#FF3B3030' : '#272E3B' }}>
            <div className="text-[10px] uppercase tracking-wider font-medium mb-2 flex items-center" style={{ color: '#6B7A90' }}>Agent Connectivity<InfoTip description="Site-controller agent health. Offline agents store data locally and risk reporting gaps until they reconnect." /></div>
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-8 rounded border overflow-hidden" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-distributor-table">
          <div className="px-4 py-2.5 border-b flex items-center gap-2" style={{ borderColor: '#272E3B' }}>
            ...
            <span className="font-heading text-sm font-semibold flex items-center" style={{ color: '#E8ECF1' }}>Distributor Compliance Matrix<InfoTip description="Per-distributor scorecard. Click any row to drill into that licensee's 30-day trend and active exceptions." /></span>
          </div>
          <div className="grid grid-cols-12 gap-1 px-4 py-2 text-[9px] uppercase tracking-wider font-medium border-b" style={{ color: '#6B7A90', borderColor: '#272E3B' }}>
            <div className="col-span-2 flex items-center">Distributor<InfoTip description="Name of the licensed machine owner/service company." /></div><div className="col-span-1 flex items-center">License<InfoTip description="State-issued gaming license number." /></div><div className="col-span-1 flex items-center">Devices<InfoTip description="Machines operating under this distributor." /></div>
            <div className="col-span-1 flex items-center">NOR<InfoTip description="30-day Net Operating Revenue (Coin In minus Coin Out)." /></div><div className="col-span-1 flex items-center">Tax<InfoTip description="30-day state tax/fees owed on this NOR." /></div><div className="col-span-1 flex items-center">Hold%<InfoTip description="NOR divided by Coin In. Outliers may indicate machine tuning issues." /></div>
            <div className="col-span-1 flex items-center">Integrity<InfoTip description="Pass rate on software signature checks for this distributor's machines." /></div><div className="col-span-1 flex items-center">Exceptions<InfoTip description="Active unresolved alerts on this distributor's machines." /></div><div className="col-span-2 flex items-center">Score<InfoTip description="Composite compliance score for this distributor — weights integrity, statutory coverage, and exception backlog." /></div><div className="col-span-1"></div>
          </div>
        ...
        <div className="col-span-4 space-y-3">
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-county-breakdown">
            <div className="text-[10px] uppercase tracking-wider font-medium mb-2 flex items-center" style={{ color: '#6B7A90' }}>Events by County<InfoTip description="Event volume by county. Some states require per-county revenue and tax reporting." /></div>
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }} data-testid="reg-exc-types">
            <div className="text-[10px] uppercase tracking-wider font-medium mb-2 flex items-center" style={{ color: '#6B7A90' }}>Active Exceptions by Type<InfoTip description="Unresolved alerts grouped by category — helps spot systemic issues across the estate." /></div>
            <div className="space-y-1">
          ...
          <div className="rounded border p-3" style={{ background: '#12151C', borderColor: '#272E3B' }}>
            <div className="text-[10px] uppercase tracking-wider font-medium mb-2 flex items-center" style={{ color: '#6B7A90' }}>Integrity by Trigger<InfoTip description="Pass/fail breakdown grouped by what caused the check — scheduled sweep, boot, manual, or post-incident." /></div>
            <div className="space-y-1">
      ...
      {d.integrity_compliance?.failed_checks?.length > 0 && (
        <div className="rounded border overflow-hidden" style={{ background: '#12151C', borderColor: '#FF3B3030' }} data-testid="reg-failed-integrity">
          <div className="px-4 py-2.5 border-b flex items-center gap-2" style={{ borderColor: '#272E3B', background: 'rgba(255,59,48,0.03)' }}>
            ...
            <span className="font-heading text-sm font-semibold flex items-center" style={{ color: '#FF3B30' }}>Integrity Violations Requiring Investigation<InfoTip description="Machines whose reported software signature did not match the approved reference. Possible tampering — must be examined before the machine returns to play." /></span>
          </div>
          <div className="grid grid-cols-12 gap-2 px-4 py-2 text-[9px] uppercase tracking-wider font-medium border-b" style={{ color: '#6B7A90', borderColor: '#272E3B' }}>
            <div className="col-span-2 flex items-center">Time<InfoTip description="When the check ran." /></div><div className="col-span-2 flex items-center">Device<InfoTip description="The machine that failed the check." /></div><div className="col-span-1 flex items-center">Trigger<InfoTip description="What caused the check (scheduled, boot, manual)." /></div><div className="col-span-2 flex items-center">Software<InfoTip description="Firmware version reported by the machine." /></div><div className="col-span-2 flex items-center">Expected Sig<InfoTip description="The SHA-256 fingerprint the lab-approved build should produce." /></div><div className="col-span-2 flex items-center">Reported Sig<InfoTip description="The fingerprint the machine actually reported. Mismatch = flag." /></div><div className="col-span-1 flex items-center">Action<InfoTip description="What the system did automatically (e.g. DEVICE_DISABLED)." /></div>
          </div>
