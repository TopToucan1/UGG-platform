  ...
  return (
    <div data-testid="proxy-control" className="space-y-4">
      <h1 className="font-heading text-2xl font-bold flex items-center gap-3" style={{ color: '#F0F4FF' }}>
        ...
        <InfoTip label="Proxy Mode" description="Lets the platform sit between an EGM and its upstream host system to observe or modify protocol traffic. Used for debugging integrations, validating schemas, and simulating bad-network conditions in a safe way." />
      </h1>

      {/* Config */}
      <div className="rounded-lg border p-4 space-y-3" style={{ background: '#0C1322', borderColor: '#1A2540' }}>
        <div className="grid grid-cols-3 gap-3">
          <div><label className="flex items-center text-[9px] uppercase tracking-wider mb-1" style={{ color: '#4A6080' }}>Target Host<InfoTip description="The upstream host (usually the casino CMS or G2S host) that traffic should be forwarded to." /></label><input value={targetHost} onChange={e => setTargetHost(e.target.value)} className="w-full px-3 py-2 rounded text-xs font-mono outline-none" style={{ background: '#111827', border: '1px solid #1A2540', color: '#F0F4FF' }} /></div>
          <div><label className="flex items-center text-[9px] uppercase tracking-wider mb-1" style={{ color: '#4A6080' }}>Listen Port<InfoTip description="Port the proxy will open on this server. Your EGMs should be pointed at this port instead of the real host." /></label><input type="number" value={listenPort} onChange={e => setListenPort(+e.target.value)} className="w-full px-3 py-2 rounded text-xs font-mono outline-none" style={{ background: '#111827', border: '1px solid #1A2540', color: '#F0F4FF' }} /></div>
          <div className="flex items-end"><button data-testid="start-proxy-btn" onClick={startProxy} className="w-full py-2 rounded text-xs font-semibold" style={{ background: '#8B5CF6', color: '#F0F4FF' }}><Play size={14} className="inline mr-1" /> Start Proxy</button><InfoTip description="Spin up a new proxy instance with the settings above. It will start capturing traffic immediately." /></div>
        </div>

        {/* Filters */}
        <div className="flex items-center text-[10px] uppercase tracking-wider font-medium" style={{ color: '#4A6080' }}>Disruptive Filters<InfoTip description="Rules that intentionally mess with traffic so you can test how the system reacts. DROP = silently delete the message. DELAY = hold it. CORRUPT = mangle the payload. DUPLICATE = send it twice. Only use against test hosts." /></div>
        <div className="flex gap-2">
          ...
          <InfoTip description="Add this filter to the disruptive rules list that will be attached when you start the proxy." />
        </div>
        {filters.map((f, i) => (
        ...
        <div key={p.id} className="rounded-lg border p-4" style={{ background: '#0C1322', borderColor: p.status === 'RUNNING' ? '#8B5CF630' : '#1A2540' }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <InfoTip description="Green pulsing dot = proxy is running and actively handling traffic. Grey = stopped." />
            ...
            <div className="flex gap-2">
              ...
              <InfoTip description="Fire a synthetic G2S message through this proxy to confirm routing, schema validation, and channel fan-out are working." />
              ...
              <InfoTip description="Shut down this proxy instance. EGMs pointed at it will lose connectivity until traffic is redirected." />
            </div>
          </div>
          <div className="grid grid-cols-4 gap-3 text-xs font-mono">
            <div className="flex items-center" style={{ color: '#4A6080' }}>Captured: <span style={{ color: '#F0F4FF' }}>{p.messages_captured}</span><InfoTip description="Total messages the proxy has seen since it was started." /></div>
            <div className="flex items-center" style={{ color: '#4A6080' }}>Forwarded: <span style={{ color: '#00D97E' }}>{p.messages_forwarded}</span><InfoTip description="Messages that passed through cleanly to the upstream host." /></div>
            <div className="flex items-center" style={{ color: '#4A6080' }}>Dropped: <span style={{ color: '#FF3B3B' }}>{p.messages_dropped}</span><InfoTip description="Messages that a disruptive filter discarded. Should be zero in production." /></div>
            <div className="flex items-center" style={{ color: '#4A6080' }}>Schema Violations: <span style={{ color: '#FFB800' }}>{p.schema_violations}</span><InfoTip description="Messages that did not match the expected G2S schema. High numbers usually mean an integration bug on one side." /></div>
          </div>
        </div>
      ))}
